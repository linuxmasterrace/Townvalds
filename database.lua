function ConfigureDatabase()
	...
	local sqlCreate = {
		...
		"CREATE TABLE IF NOT EXISTS towns (town_id INTEGER NOT NULL PRIMARY KEY AUTOINCREMENT, town_name STRING NOT NULL UNIQUE, town_owner STRING NOT NULL UNIQUE, nation_id INTEGER, town_explosions_enabled INTEGER NOT NULL DEFAULT 0, town_pvp_enabled INTEGER NOT NULL DEFAULT 0, town_mobs_enabled INTEGER NOT NULL DEFAULT 0, town_fire_enabled INTEGER NOT NULL DEFAULT 0, town_spawnX INTEGER NOT NULL, town_spawnY INTEGER NOT NULL, town_spawnZ INTEGER NOT NULL, town_spawnWorld INTEGER NOT NULL, FOREIGN KEY (nation_id) REFERENCES nations(nation_id) ON UPDATE CASCADE ON DELETE SET NULL)",
