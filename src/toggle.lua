...
function OnExploding(World, ExplosionSize, CanCauseFire, X, Y, Z, Source, SourceData)
	...
	if ExecuteStatement(town_sql, town_parameters)[1] ~= nil then
		...
		local entity_sql = "SELECT * FROM towns WHERE town_explosions_enabled = 0 and town_id = ?";
		local entity_paramaters = {town_id};
		local result = ExecuteStatement(entity_sql, entity_paramters)[1];
		
		if result == nil then
			return true
		end

		if result == town_id then
			exData[5] = 'disabled'
		end
	end
end
		...
		if exData[1] ~= nil then -- Test for explosion data
			...
			if Entity:IsPickup() or Entity:IsFallingBlock() then
				if InsideArea(position.x, exData[1] + exData[4], exData[1] - exData[4]) then
					if InsideArea(position.y, exData[2] + exData[4], exData[2] - exData[4] - 3) then
