function CreateDatabase()
	...
	local sqlCreate = {
		...
		"CREATE TABLE IF NOT EXISTS nations (nation_id INTEGER NOT NULL PRIMARY KEY AUTOINCREMENT UNIQUE, nation_name STRING NOT NULL UNIQUE, nation_capital INTEGER NOT NULL UNIQUE)",
		"CREATE TABLE IF NOT EXISTS towns (town_id INTEGER NOT NULL PRIMARY KEY AUTOINCREMENT UNIQUE, town_name STRING NOT NULL UNIQUE, town_owner STRING NOT NULL UNIQUE, nation_id INTEGER, town_explosions_enabled INTEGER NOT NULL, town_pvp_enabled INTEGER NOT NULL, town_mobs_enabled INTEGER DEFAULT 0, FOREIGN KEY (nation_id) REFERENCES nations(nation_id) ON UPDATE CASCADE)",
		...
		"CREATE TABLE IF NOT EXISTS residents (player_uuid STRING NOT NULL PRIMARY KEY UNIQUE, player_name STRING NOT NULL UNIQUE, town_id INTEGER, town_rank STRING, last_online INTEGER, FOREIGN KEY (town_id) REFERENCES towns(town_id) ON UPDATE CASCADE ON DELETE SET NULL)",
